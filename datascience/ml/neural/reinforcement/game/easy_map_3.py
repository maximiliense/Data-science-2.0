...
class EasyMap(AbstractGame):
    def __init__(self, nb_obstacles=30, max_height=500, max_width=500, nb_coup_max=1000):
        super().__init__()
    ...
    def direction(self):
        dir = np.zeros([8])
        bottom = self.position[0] - self.target[0] > 0
        top = self.position[0] - self.target[0] < 0
        right = self.position[1] - self.target[1] > 0
        left = self.position[1] - self.target[1] < 0
        if top and left:
            dir[0] = 1
            ...
            dir[4] = 1
        elif bottom and not right and not left:
            dir[5] = 1
        elif bottom and left:
            dir[6] = 1
        elif not bottom and not top and left:
            dir[7] = 1
        return dir
    ...
    def get_state(self):
        return torch.from_numpy(self.game[:, self.position[0] - 25:self.position[0] + 25,
                                self.position[1] - 25:self.position[1] + 25]).float(), torch.from_numpy(
