...
class InceptionEnvCoocs(nn.Module):
    ...
    def __init__(self, n_labels=3336, n_input=80, dropout=0.5, last_layer=False, config=0):
        ...
        # co-occurrences
        if config == 1:
            self.fc_cooc_1 = nn.Linear(n_labels, 32)
            self.bn_cooc_1 = nn.BatchNorm1d(32)
            self.fc_cooc_2 = nn.Linear(32, 32)
        else:
            self.fc_cooc_1 = nn.Linear(n_labels, 16)
            self.bn_cooc_1 = nn.BatchNorm1d(16)

        # fusion
        if config == 1:
            self.bn_fusion = nn.BatchNorm1d(2080)
            self.fc = nn.Linear(2080, n_labels)
        else:
            self.bn_fusion = nn.BatchNorm1d(2064)
            self.fc = nn.Linear(2064, n_labels)
    ...
    def forward(self, x, x_cooc):
        ...
        # n_labels
        x_cooc = self.bn_cooc_1(self.fc_cooc_1(x_cooc))

        if self.config == 1:
            # 32
            x_cooc = F.relu(x_cooc)

            # 32
            x_cooc = self.fc_cooc_2(x_cooc)

        # fusion

        # 2048 + 16 = 2064
        x = torch.cat((x, x_cooc), dim=1)
