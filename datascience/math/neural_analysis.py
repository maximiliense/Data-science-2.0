...
def compute_filters(model, data, nb_filters=100, include_logit=True):
    ...
    batch = torch.stack([choice(data)[0] for _ in range(nb_filters)])
    ...
    for layer in range(1, len(model_instance) + include_logit):
        ...
        for j in range(nb_filters):
            ...
            if _is_convolutive(output):
                output[j, select_filter, select_filter_output].backward(retain_graph=True)
            else:
                output[j, select_filter].backward(retain_graph=True)

            # the tensor must be cloned
            arr = torch.flatten(batch.grad[j].detach()).clone().cpu().numpy()
